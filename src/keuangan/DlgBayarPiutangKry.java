 ...
 * @author dosen
 */
public final class DlgBayarPiutangKry extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgBayarPiutangKry(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Bayar Piutang Karyawan]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        Keterangan.setBounds(89, 100, 290, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRawat.getText().trim().equals("")){
            ...
            if(Sequel.menyimpantf("bayar_piutang","?,?,?,?,?,?,?","Pembayaran",7,new String[]{
                    Valid.SetTgl(Tanggal.getSelectedItem()+""),"000000",Cicilan.getText(),
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgBayarPiutangKry dialog = new DlgBayarPiutangKry(new javax.swing.JFrame(), true);
    ...
    public void tampil() {
        ...
        try{    
            ps=koneksi.prepareStatement(
                    ...
                    "bayar_piutang.catatan, bayar_piutang.no_rawat,bayar_piutang.kd_rek,bayar_piutang.kd_rek_kontra, SUBSTRING_INDEX(piu.catatan,\"-\",1) AS nik,SUBSTRING_INDEX(piu.catatan,\"-\",-1) AS nm_kry from bayar_piutang "+
                    "INNER JOIN piutang piu ON piu.nota_piutang = bayar_piutang.no_rawat inner join pasien on bayar_piutang.no_rkm_medis=pasien.no_rkm_medis where "+
    ...
    public void setData(String norawat,String norm,String nama){
        ...
        Keterangan.setText(now.substring(11, 13) + ":" + now.substring(14, 16) + ":" + now.substring(17, 19)+"|"+Sequel.cariIsi("SELECT ptg.nama FROM petugas ptg WHERE ptg.nip =?", akses.getkode()));
        ...
        if(Sequel.cariInteger("select count(nota_piutang) from piutang where nota_piutang=?",NoRawat.getText())==1){
            ...
            sisapiutang=Sequel.cariIsiAngka("SELECT ifnull(SUM(piutang.sisapiutang),0) FROM piutang where piutang.nota_piutang=?",NoRawat.getText())
                        -Sequel.cariIsiAngka("SELECT ifnull(SUM(bayar_piutang.besar_cicilan),0) FROM bayar_piutang where bayar_piutang.no_rawat=?",NoRawat.getText());
            Sisa.setText(Valid.SetAngka(Valid.roundUp(sisapiutang,100)));
            Cicilan.setText(Valid.SetAngka(Valid.roundUp(sisapiutang,100)));
            status="obat";
        }else{
            Valid.loadCombo(AkunPiutang,"select nama_bayar from detail_piutang_pasien where no_rawat='"+NoRawat.getText()+"'");
            kontraakun=Sequel.cariIsi("select kd_rek from akun_piutang where nama_bayar=?",AkunPiutang.getSelectedItem().toString());
